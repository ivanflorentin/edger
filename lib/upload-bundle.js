...
const uploadZipFile = function (folderPath, config) {
    ...
    let fileToSend = fs.createReadStream(folderPath + '/bundle.zip')
    ...
    request(options, function (error, response, body) {
        ...
        if (response.statusCode == 200) {
            log(colors.info("\n==================================================================================="))
            log(colors.info("Upload Succesfully"))
            log(colors.info("===================================================================================\n"))
        ...
        cleanDir(folderPath + '/bundle.zip')
    })
}
...
const streamZipFile = function (config, folderToZip) {
    ...
    output.on('close', function () {
        log(colors.warn("\n==================================================================================="))
        log(colors.warn("bundle is ready to upload"))
        log(colors.warn("===================================================================================\n"))

        log(colors.verbose(archive.pointer() + ' total bytes zipped'))

        log(colors.warn("\n==================================================================================="))
        log(colors.warn("upload bundle to APIGEE begins..."))
        log(colors.warn("===================================================================================\n"))

        uploadZipFile(folderToZip, config)
    })
    ...
    archive
        .directory(folderToZip, 'apiproxy')
        .finalize()
}
